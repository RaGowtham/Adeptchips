...
{
    char logbuffer[550] = { 0};
    int fd;
    switch (type)
    {
	case ERROR:
	    sprintf(logbuffer,"ERROR: %s: %s (errno=%d pid=%d socket=%d)\n",title, description, errno, getpid(), socket_fd);
	    break;
	default:
	    sprintf(logbuffer,"INFO: %s: %s (pid=%d socket=%d)\n",title, description, getpid(), socket_fd);
	    break;
    }
    if((fd = open("logger.txt", O_CREAT | O_WRONLY | O_APPEND,0644)) >= 0) {
...
{
    int fd = open("logger.txt", O_TRUNC);
    if (argc != 2)
    {
    ...
    if (argc == 2 )
    {
	dwebserver(atoi(argv[1]), &send_response, &default_logger);
    }
}
...
void send_response(struct hitArgs *args, char *path, char *request_body, http_verb type)
{
    ...
    {
	return send_gpio(args, path, request_body);
